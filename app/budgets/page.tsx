...
export default function BudgetsPage() {
  const { budgets, transactions, addBudget, deleteBudget, currency, lang } = useFinanceStore();
  const tr = t(lang);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold" style={{ color: "var(--foreground)" }}>{tr.budgetsTitle}</h1>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="flex-1 w-full">
              <label className="text-xs font-medium mb-1 block" style={{ color: "var(--muted)" }}>{tr.category}</label>
            ...
            <div className="flex-1 w-full">
              <label className="text-xs font-medium mb-1 block" style={{ color: "var(--muted)" }}>{tr.monthlyLimit}</label>
              <input type="number" min="1" placeholder="0.00" value={form.limit}
                onChange={(e) => setForm((f) => ({ ...f, limit: e.target.value }))}
                className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                style={{ background: "var(--background)", border: "1px solid var(--border)", color: "var(--foreground)" }} />
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              <button type="button" onClick={handleAdd} className="flex-1 sm:flex-none px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium">{tr.add}</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {monthBudgets.map((budget, i) => {
          ...
          return (
            <motion.div key={budget.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "var(--muted)" }}>{formatCurrency(spent, currency)} / {formatCurrency(budget.limit, currency)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {over80 && <AlertTriangle size={15} style={{ color: over100 ? "#ef4444" : "#f59e0b" }} />}
                  <button onClick={() => deleteBudget(budget.id)} style={{ color: "var(--muted)" }}><Trash2 size={14} /></button>
                </div>
              </div>
              ...
              <div className="flex justify-between mt-2">
                <span className="text-xs" style={{ color: "var(--muted)" }}>
                  {over100 ? `⚠️ ${tr.overBudget}` : over80 ? `⚠️ ${tr.almostFull}` : `${(100 - pct).toFixed(0)}% ${tr.remaining}`}
                </span>
      ...
      {monthBudgets.length === 0 && (
        <div className="text-center py-24" style={{ color: "var(--muted)" }}>
          ...
          <p className="text-sm">{tr.noBudgets}</p>
        </div>
      )}
    </div>
  );
