    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--muted)" }}>Envelope budgeting for {format(new Date(), "MMMM yyyy")}</p>
        </div>
        <motion.button
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form
            ...
            onSubmit={handleAdd}
            className="rounded-2xl p-6 mb-6 flex gap-4 items-end"
          ...
          >
            ...
            <button type="submit" className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium">Add</button>
            <button type="button" onClick={() => setShowForm(false)} className="px-5 py-2.5 rounded-xl text-sm font-medium"
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  {over80 && <AlertTriangle size={16} style={{ color: over100 ? "#ef4444" : "#f59e0b" }} />}
      ...
      {monthBudgets.length === 0 && (
        <div className="text-center py-24" style={{ color: "var(--muted)" }}>
          ...
          <p className="text-sm">No budgets set for this month. Add one to get started.</p>
        </div>
      )}
    </div>
  );
