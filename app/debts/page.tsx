...
import { useFinanceStore, CURRENCIES, CURRENCY_SYMBOLS, DebtDirection, Debt } from "@/store/useFinanceStore";
...
export default function DebtsPage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.personName || !form.amount || !form.reason) return;
    addDebt({
      ...form,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>{owedToMe.length} active debt{owedToMe.length !== 1 ? "s" : ""}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>{iOwe.length} active debt{iOwe.length !== 1 ? "s" : ""}</p>
        </motion.div>
      </div>
        ...
        {showForm && (
          <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { label: "Person's Name", key: "personName", type: "text", placeholder: "John Doe", required: true },
                { label: "Reason / Description", key: "reason", type: "text", placeholder: "e.g. Dinner, Loan", required: true },
                ...
                { label: "Due Date (optional)", key: "dueDate", type: "date", placeholder: "", required: false },
                { label: "Note (optional)", key: "note", type: "text", placeholder: "Any extra details", required: false },
                    ...
                    <div className="mt-3">
                      <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
                        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }}
                          ...
                          style={{ background: debt.status === "settled" ? "#22c55e" : "#6366f1" }} />
                      </div>
                      <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>
                        {formatCurrency(debt.amountPaid, debt.currency)} paid · {pct.toFixed(0)}%
                      </p>
                    </div>
                  )}
                  ...
                  {daysLeft !== null && debt.status !== "settled" && (
                    <div className="flex items-center gap-1.5 mt-2">
                      {isOverdue
                        ...
                        : <Clock size={13} style={{ color: daysLeft <= 7 ? "#d97706" : "var(--muted)" }} />}
                      ...
                      {paymentDebtId === debt.id ? (
                        <div className="flex gap-2 w-full flex-wrap">
                          ...
                          <button onClick={() => handlePayment(debt.id)}
                            className="px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-medium">Record</button>
