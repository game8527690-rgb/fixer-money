...
export default function AnalyticsPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl md:text-2xl font-bold" style={{ color: "var(--foreground)" }}>Analytics</h1>
        <p className="text-xs md:text-sm mt-1" style={{ color: "var(--muted)" }}>Your financial patterns</p>
      </div>

      {/* Summary — 1 col mobile, 3 desktop */}
      ...
      <div className="rounded-2xl p-4 md:p-6 mb-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
        <h3 className="font-semibold mb-4 text-sm md:text-base" style={{ color: "var(--foreground)" }}>Monthly Cash Flow</h3>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={trend}>
            ...
            <Legend />
            <Line type="monotone" dataKey="income" stroke="#22c55e" strokeWidth={2.5} dot={{ r: 3 }} name="Income" />
            <Line type="monotone" dataKey="expense" stroke="#ef4444" strokeWidth={2.5} dot={{ r: 3 }} name="Expenses" />
          </LineChart>
        </ResponsiveContainer>
      </div>

      {/* Donut + Heatmap — stacked on mobile */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-2xl p-4 md:p-6" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <h3 className="font-semibold mb-4 text-sm md:text-base" style={{ color: "var(--foreground)" }}>Expense Breakdown</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
        ...
        <div className="rounded-2xl p-4 md:p-6" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <h3 className="font-semibold mb-4 text-sm md:text-base" style={{ color: "var(--foreground)" }}>Spending by Day</h3>
